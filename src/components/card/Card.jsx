...
function Card({
  ...
  if (activeTask) {
    return (
      <TaskDetail 
        ...
        onUpdateTask={async (taskId, title, description, startDate, status) => {
          const updated = await onUpdateTask(taskId, title, description, startDate, status);
  ...
  return (
    <div>
      ...
      {tasks.length > 0 ? (
        <TaskList tasks={tasks} onEnterTask={(task) => setActiveTask(task)} />
      ) : (
        !loadingTasks && <p>Задач нет</p>
      )}
      
      <button onClick={() => setIsEditingCard(true)}>редактировать эту карточку</button>
      <button onClick={handleDeleteCardClick}>удалить эту карточку</button>
    </div>
  );
}
